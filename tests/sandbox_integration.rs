...
use tbank_nt_community::{
    common::{
        consts::{DEFAULT_REQUEST_TIMEOUT, RUB_CURRENCY, SANDBOX_ENDPOINT, SANDBOX_TOKEN_ENV},
    ...
    grpc::{
        ...
        connect_channel,
        generated::{
            self, CandleInterval, GetAccountsRequest, GetCandlesRequest, GetLastPricesRequest,
            GetOrderBookRequest, GetOrdersRequest, GetStopOrdersRequest, GetTradingStatusRequest,
            InstrumentIdType, InstrumentRequest, LastPriceType, OrderDirection, PortfolioRequest,
            PositionsRequest, SandboxPayInRequest, SecurityTradingStatus, StopOrderStatusOption,
            SubscribeLastPriceRequest, SubscriptionAction, SubscriptionStatus,
...
impl CleanupGuard {
    ...
    async fn assert_no_active_orders(&mut self) -> Result<()> {
        ...
        for order_id in &self.order_ids {
                ...
                !active_ids.contains(&order_id.as_str()),
                "cleanup left active order_id={order_id}"
            );
        }
        Ok(())
    }
    ...
    async fn assert_no_active_stop_orders(&mut self) -> Result<()> {
        ...
        let active_ids: Vec<_> = active
            ...
            .collect();
        for stop_order_id in &self.stop_order_ids {
            ensure!(
                !active_ids.contains(&stop_order_id.as_str()),
                "cleanup left active stop_order_id={stop_order_id}"
            );
        }
        Ok(())
    }
...
impl MarketFillCleanupGuard {
    ...
    async fn cleanup(&mut self) -> Result<()> {
        ...
        let current = sandbox_position_quantity(
            &self.env,
            &mut self.clients,
            &self.account_id,
            &instrument_uid,
        ...
        .await?;
            ...
            "market-fill cleanup sell did not fill: status={} order_id={} residual_position={} lot={}",
            sell.order_status,
            sell.venue_order_id,
            current,
            lot
        ...
        let after = sandbox_position_quantity(
            &self.env,
            &mut self.clients,
            &self.account_id,
            &instrument_uid,
...
async fn sandbox_position_quantity(
    ...
    for attempt in 0..3_u64 {
        ...
        {
            Ok(response) => {
                return Ok(stock_quantity_from_positions(
                    &response.into_inner(),
                    instrument_uid,
                ));
            }
...
async fn sandbox_market_fill() -> Result<()> {
    ...
    let before =
        sandbox_position_quantity(&env, &mut clients, &account_id, &instrument.instrument_uid)
            .await?;
    ...
    let after_buy =
        ...
        after_buy >= before + i64::from(instrument.lot),
